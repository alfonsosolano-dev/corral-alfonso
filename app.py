...
# Configuración
st.set_page_config(page_title="Corral Maestro PRO", page_icon="🐓", layout="wide")
...
# Menú
menu = st.sidebar.selectbox("MENÚ PRINCIPAL", ["🏠 Dashboard","🐣 Alta de Lotes","📊 Ranking Lotes","📈 Predicción Huevos","📜 Histórico","💾 Backup","🧾 Informe Mensual"])

# Dashboard
if menu=="🏠 Dashboard":
    st.title("🏠 Panel Corral Maestro PRO")
...
# Ranking Lotes
    ...
    st.title("🏆 Ranking Lotes por Producción")
    df_rank = ranking_lotes(produccion)
    st.dataframe(df_rank)

# Predicción Huevos
elif menu=="📈 Predicción Huevos":
    st.title("🔮 Predicción de Huevos (Media Móvil)")
    df_pred = media_movil_produccion(produccion)
    if not df_pred.empty:
        st.line_chart(df_pred[["huevos","media_movil"]].set_index(produccion["fecha"]))
